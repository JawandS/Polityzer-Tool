        ...
        for candidate, candidate_office, website in self.candidates:
            ...
            for webpage in CandidateUtils.get_webpages(candidate, candidate_office):
                if os.path.isdir(webpage):
                    _Logger.debug(f"Skipping {webpage} as it is a directory.")
                    continue
                links_with_texts = LinkExtractor.get_links_with_texts(webpage)
                for link in links_with_texts:
                    for link_text, link_url in link.items():
                        for word in self.bag_of_words:
                            if word.lower() in link_text.lower() or word.lower() in link_url.lower():

                                _Logger.debug(f"Privacy policy link using keyword {word}: {link} in {webpage}")
                                privacy_flag = True
                                if link not in privacy_links:
                                    privacy_links.append(link)
                                privacy_keywords.add(word)
                    ...
                    #     _Logger.debug(f"Privacy policy link found: {privacy_link[0]} in {webpage}")
            ...
            candidate_map[candidate] = {
                ...
                "website": website,
                "privacy_links_with_link_text": privacy_links,
